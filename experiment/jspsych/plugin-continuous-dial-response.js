var jsPsychContinuousDialResponse = (function (jspsych) {
    'use strict';
  
    const info = {
        name: "continuous-dial-response",
        parameters: {
     ...
     *
     * jsPsych plugin for showing an image stimulus and getting a continuous slider response
     *
     * @author Ben and Sophie Lloyd-Hurwitz
     */
    class ImageSliderResponsePlugin {
        ...
        trial(display_element, trial) {
            ...
            // Ben and Sophie Lloyd-Hurwitz edits begin

            var first_click_time = null; // Stores the time of the first click
            var start_clicked = false; // Checks if we have clicked yet
            // Starts countdown for end of trial
            const start_trial = () => {
                this.jsPsych.pluginAPI.setTimeout(() => {
                    end_trial();
                }, trial.trial_duration);
                if (!start_clicked) {
                    first_click_time = performance.now();   
                    start_clicked = true;
                }
                };
            // Calls start trial on first click
            display_element
                .querySelector("#jspsych-image-slider-response-response")
                .addEventListener("mousedown", start_trial);

            // Creates list to store of response times and response
            const ratings = [];
            const update_rating = () => {
                // Only record data after first click
                if (start_clicked) {
                var time_rating = {rt: performance.now() - first_click_time,
                                   response: display_element.querySelector("#jspsych-image-slider-response-response").valueAsNumber};
                ratings.push(time_rating);
                }
            };
            display_element
                .querySelector("#jspsych-image-slider-response-response")
                .addEventListener("mousemove", update_rating);
            ...
            // SOPHIE: ask Kate, what should we do if people never click?
            // end trial if trial_duration if no clicks after 10 times length
            if (trial.trial_duration !== null) {
                this.jsPsych.pluginAPI.setTimeout(() => {
                    end_trial();
                }, 10*trial.trial_duration);
            }
        }
